...
class Gesture {
    ...
    static func startGestureFromTouches(
...
final class SingleFingerGesture: Gesture {
    ...
    override init(_ touches: Set<UITouch>, _ sceneView: ARSCNView, _ virtualObject: VirtualObject) {
        ...
        self.initialTouchLocation = touch.location(in: sceneView)
        self.latestTouchLocation = initialTouchLocation
        ...
        for result in results {
            if VirtualObject.isNodePartOfVirtualObject(result.node) {
                self.firstTouchWasOnObject = true
                break
            }
        }
    }
...
final class TwoFingerGesture: Gesture {
    ...
    override init(_ touches: Set<UITouch>, _ sceneView: ARSCNView, _ virtualObject: VirtualObject) {
        ...
        self.firstTouch = currentTouches[currentTouches.index(currentTouches.startIndex, offsetBy: 0)]
        self.secondTouch = currentTouches[currentTouches.index(currentTouches.startIndex, offsetBy: 1)]
        ...
        self.objectBaseScale = CGFloat(virtualObject.scale.x)
        ...
        self.allowScaling = (firstTouchWasOnObject || objectBaseScale < 0.1)

        let loc2ToLoc1 = loc1 - loc2
        self.initialDistanceBetweenFingers = loc2ToLoc1.length()

        let midPointToLoc1 = loc2ToLoc1 / 2
        self.initialFingerAngle = atan2(Float(midPointToLoc1.x), Float(midPointToLoc1.y))
