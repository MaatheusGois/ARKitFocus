...
class FocusSquare: SCNNode {
    ...
    private func updateTransform(for position: SCNVector3, camera: ARCamera?) {
        ...
        if let average = recentFocusSquarePositions.average {
            ...
            setUniformScale(scaleBasedOnDistance(camera: camera))
        }
        ...
        if let camera {
            ...
            rotation = SCNVector4Make(0, 1, 0, angle)
        }
    }
    ...
    private func scaleBasedOnDistance(camera: ARCamera?) -> Float {
        if let camera {
            let distanceFromCamera = (worldPosition - SCNVector3.positionFromTransform(camera.transform)).length()
    ...
    private func open() {
        ...
        segments?[0].open(direction: .left, newLength: sideLengthForOpenSquareSegments)
        segments?[1].open(direction: .right, newLength: sideLengthForOpenSquareSegments)
        segments?[2].open(direction: .up, newLength: sideLengthForOpenSquareSegments)
        segments?[3].open(direction: .up, newLength: sideLengthForOpenSquareSegments)
        segments?[4].open(direction: .down, newLength: sideLengthForOpenSquareSegments)
        segments?[5].open(direction: .down, newLength: sideLengthForOpenSquareSegments)
        segments?[6].open(direction: .left, newLength: sideLengthForOpenSquareSegments)
        segments?[7].open(direction: .right, newLength: sideLengthForOpenSquareSegments)
    ...
    private var fillPlane: SCNNode? {
        childNode(withName: "fillPlane", recursively: true)
    }
...
class FocusSquareSegment: SCNNode {
    ...
    func open(direction: Direction, newLength: CGFloat) {
        guard let p = geometry as? SCNPlane else {
            return
        }
        ...
        switch direction {
        case .left:
            position.x -= Float(0.5 / 2 - p.width / 2)
        case .right:
            position.x += Float(0.5 / 2 - p.width / 2)
        case .up:
            position.y -= Float(0.5 / 2 - p.height / 2)
        case .down:
            position.y += Float(0.5 / 2 - p.height / 2)
        }
    }
    ...
    func close(direction: Direction) {
        ...
        switch direction {
        case .left:
            position.x -= Float(0.5 / 2 - oldLength / 2)
        case .right:
            position.x += Float(0.5 / 2 - oldLength / 2)
        case .up:
            position.y -= Float(0.5 / 2 - oldLength / 2)
        case .down:
            position.y += Float(0.5 / 2 - oldLength / 2)
        }
    }
